...
const publicFrontendUrl = "https://swiftcapital-lmtd.onrender.com";
const publicServerUrl = "https://server-4juw.onrender.com";
const payzaBaseUrl = (
...
const frontendOrigins = (process.env.FRONTEND_ORIGINS || publicFrontendUrl)
  .split(",")
  .map((origin) => origin.trim())
  .filter(Boolean);

// The deployment URLs are fixed for this integration. Only the PayZa keys
// need to be supplied as secrets; these values remain overridable for testing.
process.env.PAYZA_WEBHOOK_URL ||= `${publicServerUrl}/api/payza/webhook`;
process.env.PAYZA_REDIRECT_URL ||=
  `${publicFrontendUrl}/loan-withdrawal.html?payment=success`;
process.env.PAYZA_CANCEL_URL ||=
  `${publicFrontendUrl}/loan-withdrawal.html?payment=cancelled`;
...
async function readPayzaResponse(response) {
  ...
  if (!response.ok) {
    ...
    throw new HttpError(
      documentedStatusCodes.has(response.status) ? response.status : 502,
      "PAYZA_API_ERROR",
      upstreamMessage,
      {
...
function extractPaymentInput(body) {
  ...
  const description = getText(body.description) || "WAITAAP ONLINE ORDERS";
...
function paymentResponse(body, input) {
  ...
  if (!getText(data.payment_url)) {
    throw new HttpError(
      502,
      "INVALID_PAYZA_RESPONSE",
      "PayZa did not return the hosted payment_url required for checkout.",
      { upstreamResponse: body }
    );
  }
...
function sanitizeForLog(value, depth = 0) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (
      /secret|token|authorization|password|private.?key|api.?key|email|phone|customer|metadata/i.test(
        key
      )
    ) {
      result[key] = "[redacted]";
...
function sendError(res, error, req) {
  ...
  // HttpError is the normal path for PayZa rejections, so log it too.
  // Keep credentials and customer contact data out of Render logs.
  console.error(
    JSON.stringify({
      type: "payment_error",
      requestId,
  ...
  if (error instanceof HttpError && error.details?.upstreamStatus) {
    payload.error.upstream_status = error.details.upstreamStatus;
  }

  // The provider's message is useful to the customer and safe to return.
  // Do not return the full upstream response, which may contain customer data.
  const upstreamMessage = error.details?.upstreamResponse?.message;
  if (typeof upstreamMessage === "string" && upstreamMessage.trim()) {
    payload.error.upstream_message = upstreamMessage.trim();
  }
...
app.use((error, _req, res, _next) => {
  ...
  return sendError(res, error, _req);
});
