...
const publicBaseUrl = (
  ...
  "https://server-4juw.onrender.com"
).replace(/\/+$/, "");
const payzaWebhookUrl =
  process.env.PAYZA_WEBHOOK_URL || `${publicBaseUrl}/api/payza/webhook`;
const payzaRedirectUrl =
  process.env.PAYZA_REDIRECT_URL || `${publicBaseUrl}/payment/return`;
const payzaCancelUrl =
  process.env.PAYZA_CANCEL_URL || `${publicBaseUrl}/payment/cancel`;
...
app.get("/health", (_req, res) => {
  res.json({
    ...
    webhookConfigured: Boolean(payzaWebhookUrl),
    redirectConfigured: Boolean(payzaRedirectUrl && payzaCancelUrl),
    paymentUrls: {
      webhook: payzaWebhookUrl,
      redirect: payzaRedirectUrl,
      cancel: payzaCancelUrl,
    },
    frontendOrigins,
...
function sendPaymentResultPage(res, title, message) {
...
<html lang="en">
  <head>
    ...
    <title>${title}</title>
    <style>
      body { margin: 0; min-height: 100vh; display: grid; place-items: center; background: #081226; color: #f8fafc; font: 16px system-ui, sans-serif; }
      main { width: min(92vw, 420px); padding: 32px; border: 1px solid #263653; border-radius: 20px; background: #101d35; text-align: center; box-sizing: border-box; }
      h1 { margin: 0 0 12px; font-size: 24px; }
      p { color: #bdc9dc; line-height: 1.5; }
    </style>
  </head>
  <body><main><h1>${title}</h1><p>${message}</p><p>You can close this page and return to Swift Loan.</p></main></body>
</html>`);
}

app.get("/payment/return", (_req, res) => {
  return sendPaymentResultPage(
    res,
    "Payment Submitted",
    "Your mobile-money payment was submitted to Payza for confirmation."
  );
});
...
app.get("/payment/cancel", (_req, res) => {
  return sendPaymentResultPage(
    res,
    "Payment Cancelled",
    "The mobile-money payment was cancelled. You can return to Swift Loan and try again."
  );
});
